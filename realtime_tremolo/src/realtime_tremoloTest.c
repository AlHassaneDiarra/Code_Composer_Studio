...
#define EBSR *((volatile ioport Uint16 *)(0x1C00))  // Registre pour la sélection des broches
#define IODIR2 *((volatile ioport Uint16 *)(0x1C07)) // Configuration des directions des IO
#define IOINDATA2 *((volatile ioport Uint16 *)(0x1C09)) // Lecture des données d'entrées
...
void initGPIO()
{
    EBSR |= 0x0007;  // Sélectionne GP21-GP23 pour les I/O
    // Configure GP21 et GP22 comme entrées
    IODIR2 &= ~(1 << 5 | 1 << 6);  // GP21 et GP22 en entrée
}

Uint16 readGPIO()
{
    return (IOINDATA2 >> 5) & 0x03;  // Lecture des états GP21 et GP22
}
